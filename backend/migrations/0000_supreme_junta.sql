...
CREATE TYPE "public"."queue_status" AS ENUM('Waiting', 'Called', 'Done', 'Cancelled');--> statement-breakpoint
CREATE TYPE "public"."queue_type" AS ENUM('Reservation', 'Walkin');--> statement-breakpoint
...
CREATE TABLE "staff" (
	...
	"clinic_id" uuid,
	"code" varchar(20) NOT NULL,
	"loket_number" varchar(10) NOT NULL,
	"nik" varchar(16),
	"name" varchar(100) NOT NULL,
...
ALTER TABLE "queue_calls" ADD CONSTRAINT "queue_calls_staff_id_staff_id_fk" FOREIGN KEY ("staff_id") REFERENCES "public"."staff"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
CREATE UNIQUE INDEX "queue_number_idx" ON "queues" USING btree ("queue_number");--> statement-breakpoint
CREATE INDEX "queue_status_idx" ON "queues" USING btree ("status");--> statement-breakpoint
CREATE INDEX "queue_type_idx" ON "queues" USING btree ("queue_type");--> statement-breakpoint
CREATE INDEX "queue_clinic_idx" ON "queues" USING btree ("clinic_id");--> statement-breakpoint
CREATE INDEX "queue_date_idx" ON "queues" USING btree ("created_at");--> statement-breakpoint
CREATE INDEX "queue_status_clinic_idx" ON "queues" USING btree ("status","clinic_id");--> statement-breakpoint
CREATE MATERIALIZED VIEW "public"."dashboard_summary" AS (select COUNT(*) FILTER (WHERE status = 'Waiting') as "total_waiting", COUNT(*) FILTER (WHERE status = 'Called') as "total_called", COUNT(*) FILTER (WHERE status = 'Done') as "total_done", COUNT(*) FILTER (WHERE status = 'Cancelled') as "total_cancelled", COUNT(*) FILTER (WHERE queue_type = 'Reservation') as "total_reservation", COUNT(*) FILTER (WHERE queue_type = 'Walkin') as "total_walkin", 
...
CREATE MATERIALIZED VIEW "public"."hourly_queue_distribution" AS (select EXTRACT(HOUR FROM created_at) as "hour", COUNT(*) FILTER (WHERE queue_type = 'Reservation') as "total_reservation", COUNT(*) FILTER (WHERE queue_type = 'Walkin') as "total_walkin", COUNT(*) as "total_queues", 
      AVG(EXTRACT(EPOCH FROM (completed_at - called_at))/60) 
